...
class DependencyParser:
    ...
    def parse_repository(self, filtered_folders: List[str] = None) -> Dict[str, Node]:
        ...
        # If a target file was specified, filter the file tree to only include it
        if self.target_file:
            target_abs = os.path.abspath(self.target_file)
            target_rel = os.path.relpath(target_abs, self.repo_path)
            filtered = {
                k: v
                for k, v in file_tree.items()
                if os.path.abspath(os.path.join(self.repo_path, k)) == target_abs or k == target_rel
            }
            if filtered:
                logger.info(
                    f"target_file filter: keeping {len(filtered)} file(s) from {len(file_tree)}"
                )
                file_tree = filtered
            else:
                logger.warning(
                    f"target_file '{self.target_file}' not found in analyzed files; analyzing full repo"
                )

        call_graph_result = self.analysis_service._analyze_call_graph(file_tree, self.repo_path)
