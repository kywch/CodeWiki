...
def generate_command(
    ...
    try:
        ...
        # CLI-backend modes (Gemini/Claude Code) don't require API key config.
        # We still try to load config for optional settings (max-tokens, depth, etc.)
        # but don't enforce it.
        cli_backend_mode = use_gemini_code or use_claude_code

        config_manager = ConfigManager()
        config_loaded = config_manager.load()

        if not cli_backend_mode:
        ...
        config = config_manager.get_config() if config_loaded else None
        api_key = config_manager.get_api_key() if config_loaded else None

        if cli_backend_mode:
            logger.success("CLI backend mode — skipping API key validation")
        else:
        ...
        if verbose:
            effective_max_tokens = (
                max_tokens if max_tokens is not None else (config.max_tokens if config else 32768)
            )
            effective_max_token_per_module = (
                max_token_per_module
                if max_token_per_module is not None
                else (config.max_token_per_module if config else 36369)
            )
            effective_max_token_per_leaf = (
                max_token_per_leaf_module
                if max_token_per_leaf_module is not None
                else (config.max_token_per_leaf_module if config else 16000)
            )
            effective_max_depth = (
                max_depth if max_depth is not None else (config.max_depth if config else 2)
            )
        ...
        persistent_instructions = config.agent_instructions if config else None
        if runtime_instructions and not runtime_instructions.is_empty():
            ...
            merged = AgentInstructions(
                ...
                or (persistent_instructions.include_patterns if persistent_instructions else None),
                exclude_patterns=runtime_instructions.exclude_patterns
                or (persistent_instructions.exclude_patterns if persistent_instructions else None),
                focus_modules=runtime_instructions.focus_modules
                or (persistent_instructions.focus_modules if persistent_instructions else None),
                doc_type=runtime_instructions.doc_type
                or (persistent_instructions.doc_type if persistent_instructions else None),
                custom_instructions=runtime_instructions.custom_instructions
                or (
                    persistent_instructions.custom_instructions if persistent_instructions else None
                ),
            )
            agent_instructions_dict = merged.to_dict()
        elif persistent_instructions and not persistent_instructions.is_empty():
            agent_instructions_dict = persistent_instructions.to_dict()
        ...
        generator = CLIDocumentationGenerator(
            ...
            config={
                "main_model": config.main_model if config else None,
                "cluster_model": config.cluster_model if config else None,
                ...
                else (config.max_tokens if config else 32768),
                ...
                else (config.max_depth if config else 2),
                # CLI integrations
