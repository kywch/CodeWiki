...
class DocumentationGenerator:
    ...
    def __init__(self, config: Config, commit_id: str = None):
        ...
        # Only initialize AgentOrchestrator (which creates an OpenAI client) when
        # actually needed — CLI backend modes (Gemini, Claude Code) never use it.
        if not config.use_gemini_code and not config.use_claude_code:
            ...
            for module_path, module_name in processing_order:
                try:
                    ...
                    if self.is_leaf_module(module_info):
                        ...
                        elif self.config.use_gemini_code:
                            # Use Gemini CLI for documentation generation
                            final_module_tree = await self._process_module_with_gemini_code(
                                module_name,
                                components,
                                module_info["components"],
                                module_tree,
    ...
    async def generate_parent_module_docs(
        ...
        try:
            # Use Claude Code CLI if configured, Gemini CLI, or direct LLM call
            if self.config.use_claude_code:
                ...
                from codewiki.src.be.gemini_code_adapter import gemini_code_generate_overview

                parent_docs = gemini_code_generate_overview(prompt, self.config)
            else:
    ...
    async def _process_module_with_gemini_code(
        self,
        module_name: str,
        ...
        """
        Process a module using Gemini CLI for documentation generation.
        ...
        Returns:
            Updated module tree
        """
        from codewiki.src.be.gemini_code_adapter import gemini_code_generate_docs
        ...
        try:
            # Generate documentation using Gemini CLI
            doc_content = gemini_code_generate_docs(
            ...
            logger.error(f"Gemini CLI documentation generation failed for {module_name}: {e}")
            logger.error(f"Traceback: {traceback.format_exc()}")
            raise
    ...
    async def _process_module_with_claude_code(
        ...
        try:
            ...
            if os.path.exists(docs_path):
                logger.info(
                    f"✓ Generated documentation for {module_name} (file created by Claude Code)"
                )
            else:
